...
int uart_init(const char *device) {
    struct termios tty; // tty attributes
...
ssize_t uart_read(int uart_fd, void* buffer, int buffer_len, int null_terminate) {
    if (uart_fd < 0) {
        fprintf(stderr, "Error: Uart device was not initialized\n");
        return -1;
    }

    // if using null termination, save the buffer size for the last char
    if(null_terminate) buffer_len--;
    ...
    if(null_terminate) {
        // put an EOF char at the end
        ((char*)buffer)[n_recv] = '\0';
    }
